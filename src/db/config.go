...
// **********************
// support for PB tester. You can ignore This if you want to see source code.
// **********************
//
//
...
func (cfg *config) replicateOne(server int, cmd int, expectedServers int) (
	...
	// err_test_4002 := cfg.pbservers[v0Primary].Start(vrArgu, vrReply);
	pri.Start(vrArgu, vrReply)
	reply,_ := vrReply.Reply.(common.TestReply)
	index = reply.Index
	log.Print("reply.Index", index)
	ok := reply.OK


	if !ok {
		cfg.t.Fatalf("node-%d rejected command, index is %d, cmd is %d\n", server, index, cmd)
	}
